...
func add_frame_domekeeper_custom_raw(color_table, index_image_buffer, frame_delay, image_size, pos, transparent_index):
	last_color_table = color_table
	last_transparency_index = transparent_index
	var delay_time := calc_delay_time(frame_delay)
	add_graphic_constrol_ext(delay_time, transparent_index)
	add_image_descriptor(pos, image_size, color_table_bit_size(color_table))
	add_local_color_table(color_table)
...
func add_graphic_constrol_ext(_delay_time: float, tci: int = -1) -> void:
	...
	var packed_fields: int = 0b00000100
	if tci != -1:
		packed_fields = 0b00000101
